    ...
    * getting components and their attributes (**getComponent, getCompAttr, getOptimizationSummary**)
    ...
    def declareOptimalCostConstraint(self,pyM):
        ...
        def optimalCostConstraint(pyM):
            return (
                ...
                <= self.objectiveValue*(1+self.slack)
            )
    ...
    def declareOptimizationProblem(
        ...
        **Default arguments:**
        ...
        for key, mdl in self.componentModelingDict.items():
            ...
            utils.output("\tdeclaring variables... ", self.verbose, 0), mdl.declareVariables(self, pyM, relaxIsBuiltBinary, relevanceThreshold)
        ...
        if (
            ...
            for key, mdl in self.componentModelingDict.items():
                ...
                for optParam in optimalValueParameters:
                    i,j=convertOptimalValues(self, mdl, optParam)
                #     if j!=0:
                #         my_dict[i] =j  
                #     else:
                #         print(i)
                ...
                #         if type(y)!=type(None):
                #             y.to_excel(writer, sheet_name=f'{key[:30]}')
                    # print("------------------------------------")     
    ...
    def declareMGAOptimizationProblem(
        ...
        **Default arguments:**
        ...
        :type relevanceThreshold: float (>=0) or None
        """
        utils.output(f"MGA Iteration {iteration} {sense} .....", self.verbose, 0 )
        ...
        _t = time.time()
        self.declareOptimalCostConstraint(pyM)
        utils.output("\t\t(%.4f" % (time.time() - _t) + " sec)\n", self.verbose, 0)
        ...
        # Declare objective function by obtaining the contributions to the objective function from all modeling classes
        _t = time.time()
        self.declareMGAObjective(pyM,iteration,sense)
        utils.output("\t\t(%.4f" % (time.time() - _t) + " sec)\n", self.verbose, 0)
    ...
    def mga_optimize(
        ...
        **Default arguments:**
        ...
        self.writeSolutionsasExcels = writeSolutionsasExcels
        ...
        if self.solutions[0] is None:
            ...
            if self.writeSolutionsasExcels:
                utils.output("\nWriting optimization output to Excel files\n", self.verbose, 0)

                outdir = os.path.join(os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), "OutputData"))
